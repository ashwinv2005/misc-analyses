...
## set up world
NLDoCommandWhile("q < 0.02 or q > 0.03 or r < 0.1 or r > 0.15 or s < 0.03 or s > 0.06", "setup")
s1 = NLReport("ss1")
s2 = NLReport("SS2")
s3 = NLReport("SS3")
s4 = NLReport("SS4")
...
dailypred = data.frame(iter = rep(0,10), day = rep(0,10), number = rep(0,10), loops = rep(0,10), cap = rep(0,10), ars = rep(0,10), gud = rep(0,10), baseline = rep(0,10), lag = rep(0,10), minprev = rep(0,10), treat = treatments, srain = rain, area = s, seeds = rep(0,10), pred = rep(0,10), actpred = rep(0,10), actseeds = rep(0,10), indexp = rep(100,10))
temp = dailypred
